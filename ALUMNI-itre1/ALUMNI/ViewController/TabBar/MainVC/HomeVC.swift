...
class HomeVC: UIViewController {
  ...
  @objc func getPosts(){
    ...
    getAllPosts { tempPosts in
      for i in tempPosts {
        Firestore.firestore().collection(FSCollectionReference.users.rawValue).document(i.userID ?? "nil").getDocument { snapshot, error in
          if error == nil {
...
extension HomeVC {
  func getAllPosts(completion: @escaping ([Post])->()){
    var tempPosts = [Post]()
    Firestore.firestore().collection(FSCollectionReference.posts.rawValue).addSnapshotListener { snapshot, error in
