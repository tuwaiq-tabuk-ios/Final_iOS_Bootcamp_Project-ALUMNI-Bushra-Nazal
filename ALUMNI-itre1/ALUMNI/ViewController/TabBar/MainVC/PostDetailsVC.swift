...
class PostDetailsVC: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    db.collection(FSCollectionReference.users.rawValue).document(userID).getDocument { [self] snapshot, error in
      if error == nil {
  ...
  @IBAction func sendButtonAction(_ sender: UIButton) {
    ...
    db.collection(FSCollectionReference.comments.rawValue).document(commentID).setData([
      "commentID" : commentID,
      "commentText" : comment,
  ...
  func getPostComments() {
    ...
    db.collection(FSCollectionReference.comments.rawValue).order(by: "timestamp", descending: true).whereField("postID", isEqualTo: postID).addSnapshotListener { snapshot, error in
      if error == nil {
