  ...
  @IBAction func editButtonAction(_ sender: UIButton) {
    if toggleButton {
      ...
      if imageCheck {
          ...
          if error == nil {
            storageRef.downloadURL { [self] url, error in
              if error == nil {
                ...
                db.collection(FSCollectionReference.users.rawValue).document(userID).updateData([
        ...
        db.collection(FSCollectionReference.users.rawValue).document(userID).updateData(profileData) { [self] err in
          if err == nil {
            self.editButton.setTitle("Edit".localize(), for: .normal)
            self.editButton.isEnabled = true
            finishLoadingView()
